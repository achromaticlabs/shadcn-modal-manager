...
In Modal Manager, modals are defined as regular React components wrapped with the `ModalManager.create` HOC. This wrapper handles registration, lifecycle events, and data injection.
...
2. **`useModal()`**: This hook connects your UI component (Dialog) to the modal manager's state (open/close).
